...
crud.writeData = (fName, data, callBack) => {
  fs.open(`${crud.baseDir}/${fName}.json`, 'wx', (err1, fd) => {
    if (!err1 && fd) {
      ...
      fs.writeFile(fd, stringData, (err2) => {
        if (!err2) {
          fs.close(fd, (err3) => {
            if (!err3) {
              callBack('File successfully clossed');
            } else {
...
crud.readData = (fName, callBack) => {
  fs.readFile(`{crud.baseDir}/${fName}.json`, 'utf-8', (err, data) => {
    callBack(err, data);
  });
};

// Update existing file
crud.updateData = (fName, data, callBack) => {
  // open file for updating
  fs.open(`${crud.baseDir}/${fName}.json`, 'r+', (err1, fd) => {
    if (!err1 && fd) {
      const stringData = JSON.stringify(data);
...
crud.deleteFile = (fName, callBack) => {
  // Delete the file
  fs.unlink(`${crud.baseDir}/${fName}.json`, (err) => {
    if (!err) {
